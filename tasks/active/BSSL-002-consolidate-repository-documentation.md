...
- Local Slicer build prerequisites, configuration, clean/incremental build guidance, executable location, Reload versus rebuild guidance, and troubleshooting were preserved in `README_Stratum_Slicer_Build.md`.
...
- Current scripted-module layout and Reload guidance were preserved in `docs/slicer/scripted_module_structure.md`.
...
- Final `git status --short` showed only the approved documentation modifications/deletions plus the already-untracked active BSSL-002 task card.
...
- The documentation now points to `config/local.json` for machine-specific paths, but actual local path correctness depends on the user's private configuration.
